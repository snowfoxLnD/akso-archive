...
(function(){
  ...
  const CARD = {
    ...
    status:  'в обработке',                       // статус карты
  ...
  #cardDrop .cd-x{position:absolute;top:10px;right:10px;z-index:3;width:26px;height:26px;border-radius:50%;
    border:1px solid rgba(214,150,90,.4);background:rgba(24,14,10,.85);color:#d6a86a;font-size:.8rem;cursor:pointer;
    line-height:1;display:none;}
  #cardDrop.shut .cd-x{display:block;}
  #cardDrop .cd-x:hover{color:#fff3e2;border-color:#ffce9a;}
  ...
  function build(){
    ...
    el.querySelector('.cd-img').onclick=()=>{try{navGo('cards',document.querySelector(".nav-link[onclick*=\\"'cards'\\"]"));}catch(e){}};
    return el;
  }
  ...
  function show(){
    ...
    hideTimer=setTimeout(()=>{if(elRef)elRef.classList.add('shut');},6000);
  }
  function hide(){
    if(!elRef)return;
